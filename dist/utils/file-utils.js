...
exports.createLangChainExampleData = createLangChainExampleData;
exports.createExampleDatabase = createExampleDatabase;
 ...
 * @returns 保存的文件路径
 */
function saveTextToFile(content, filename, subfolder) {
    // 确定保存路径
    const savePath = subfolder
        ? path_1.default.join(config_1.DATA_DIR, subfolder)
        : config_1.DATA_DIR;
    // 确保目录存在
 ...
 */
function createLangChainExampleData() {
    // 确保数据目录存在
    ensureDirectoryExists(path_1.default.join(config_1.DATA_DIR, 'langchain'));
    const files = [];
    // 示例1: LangChain基本介绍
    const intro = `
# LangChain.js 简介
 ...
 * 创建示例数据库文件
 */
function createExampleDatabase() {
    // SQLite示例数据库内容
    const dbContent = `
-- 创建用户表
CREATE TABLE users (
