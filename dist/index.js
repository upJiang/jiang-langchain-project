...
const conversation_1 = require("./chains/conversation");
const agent_1 = require("./chains/agent");
const rag_1 = require("./chains/rag");
const vector_store_1 = require("./embeddings/vector-store");
const file_utils_1 = require("./utils/file-utils");
const weather_tools_1 = require("./tools/weather-tools");
 ...
 * 演示基本对话链
 */
async function demoConversationChain() {
    console.log('\n===== 演示基本对话链 =====');
    // 创建LLM实例
    const llm = llm_1.LLMFactory.createGPT3_5();
    // 创建对话链
    const chain = (0, conversation_1.createConversationChain)(llm);
    // 开始交互式对话
    console.log('开始与AI助手对话。输入 "退出" 结束对话。');
    let input = await prompt('您: ');
    while (input.toLowerCase() !== '退出') {
        // 运行对话链
        const response = await (0, conversation_1.runConversation)(chain, input);
 ...
 * 演示代理执行器
 */
async function demoAgentExecutor() {
    console.log('\n===== 演示代理执行器与工具使用 =====');
    // 创建LLM实例
    const llm = llm_1.LLMFactory.createGPT3_5();
    // 创建工具
    const tools = [
        (0, weather_tools_1.createWeatherQueryTool)(),
        (0, weather_tools_1.createWeatherComparisonTool)(),
        (0, database_tools_1.createSqlQueryTool)(),
        (0, database_tools_1.createTableInfoTool)(),
    ];
    // 创建代理执行器
    const executor = await (0, agent_1.createAgentExecutor)(tools, llm);
    // 开始交互式对话
    ...
    while (input.toLowerCase() !== '退出') {
        try {
            // 运行代理执行器
            const response = await (0, agent_1.runAgent)(executor, input);
...
async function demoRAG() {
    ...
    const files = (0, file_utils_1.createLangChainExampleData)();
    console.log(`已创建 ${files.length} 个示例文件`);
    // 创建LLM实例
    const llm = llm_1.LLMFactory.createGPT3_5();
    // 创建或加载向量存储
    console.log('正在创建向量存储...');
    const langchainDocsDir = path_1.default.join(config_1.DATA_DIR, 'langchain');
    const vectorStore = await (0, vector_store_1.loadOrCreateVectorStore)(langchainDocsDir, 'langchain-docs');
    // 创建RAG链
    console.log('正在创建RAG链...');
    const ragChain = await (0, rag_1.createRAGChain)(vectorStore, llm);
    // 开始交互式问答
    ...
    while (question.toLowerCase() !== '退出') {
        try {
            // 运行RAG链
            const answer = await (0, rag_1.runRAG)(ragChain, question);
...
async function demoDatabaseIntegration() {
    ...
    const sqlFile = (0, file_utils_1.createExampleDatabase)();
    ...
    // 关闭数据库连接
    await (0, database_tools_1.closeDbConnection)();
}
/**
 * 演示天气预报推送
 */
async function demoWeatherForecast() {
    console.log('\n===== 演示天气预报推送 =====');
    // 提示用户选择功能
    console.log('请选择功能:');
    ...
    switch (choice) {
        case '1':
            ...
            try {
                await (0, scheduler_1.runWeatherReportNow)(city);
        ...
        case '2':
            ...
            try {
                const jobId = (0, scheduler_1.scheduleDailyWeatherReport)(scheduleCity, scheduleTime);
...
async function mainMenu() {
    // 确保数据目录存在
    (0, file_utils_1.ensureDirectoryExists)(config_1.DATA_DIR);
    while (true) {
        console.log('\n===== LangChain.js 演示项目 =====');
