...
exports.createTableInfoTool = createTableInfoTool;
...
async function getDbService() {
    if (!dbService) {
        dbService = new database_1.DatabaseService('memory'); // 使用内存数据库
...
function createSqlQueryTool() {
    return new tools_1.DynamicStructuredTool({
        ...
        schema: zod_1.z.object({
            query: zod_1.z.string().describe('SQL查询语句，例如"SELECT * FROM users"'),
            params: zod_1.z.array(zod_1.z.any()).optional().describe('查询参数（可选）'),
        }),
 ...
 * @returns 数据库表信息工具实例
 */
function createTableInfoTool() {
    return new tools_1.DynamicStructuredTool({
        name: 'get_table_info',
        description: '获取指定数据库表的结构信息',
        schema: zod_1.z.object({
            table: zod_1.z.string().describe('要查询的表名'),
        }),
        func: async ({ table }) => {
            try {
